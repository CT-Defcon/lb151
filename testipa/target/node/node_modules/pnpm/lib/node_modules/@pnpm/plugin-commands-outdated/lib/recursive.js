...
exports.default = async (pkgs, params, opts) => {
    const outdatedMap = {};
    const outdatedPackagesByProject = await outdated_1.outdatedDepsOfProjects(pkgs, params, { ...opts, fullMetadata: opts.long });
...
function renderOutdatedList(outdatedMap, opts) {
    return sortOutdatedPackages(Object.values(outdatedMap))
        .map((outdatedPkg) => {
        let info = `${chalk.bold(outdated_2.renderPackageName(outdatedPkg))}
...
function dependentPackages({ dependentPkgs }) {
    return dependentPkgs
        .map(({ manifest, location }) => { var _a; return (_a = manifest.name) !== null && _a !== void 0 ? _a : location; })
        .sort()
        .join(', ');
}
